...
class RedbullTVClient(object):
    ...
    token = utils.get_json(
        "https://api.redbull.tv/v3/session?category=smart_tv&os_family=android")["token"]

    REDBULL_STREAMS = "https://dms.redbull.tv/v3/"
    REDBULL_API = "https://api.redbull.tv/v3/"
    ROOT_MENU = [
        {"title": "Live TV", "url": REDBULL_STREAMS + "linear-borb/" +
            token + "/playlist.m3u8", "is_content": True},
        {"title": "Discover", "url": REDBULL_API +
            "products/discover", "is_content": False},
        {"title": "Browse", "url": REDBULL_API +
            "products/channels", "is_content": False},
        {"title": "Events", "url": REDBULL_API +
            "products/events", "is_content": False},
    ...
    def get_element_details(self, element, element_type):
        ...
        if element.get("playable") or element.get("action") == "play":
            ...
            details["url"] = self.REDBULL_STREAMS + \
        ...
        if element.get("resources"):
            # web_pdb.set_trace()
            details["landscape"] = self.get_image_url(
                element.get("id"), element.get("resources"), "landscape")
            details["fanart"] = self.get_image_url(
                element.get("id"), element.get("resources"), "landscape")
            details["banner"] = self.get_image_url(
                element.get("id"), element.get("resources"), "banner")
            details["thumb"] = self.get_image_url(
                element.get("id"), element.get("resources"), "thumb")
            details["poster"] = self.get_image_url(
                element.get("id"), element.get("resources"), "poster")
    ...
    def get_items(self, url=None, page=1, limit=20):
        ...
        result = utils.get_json(
            url+("?", "&")["?" in url]+"limit="+str(limit)+"&offset="+str((page-1)*limit), self.token)

        if 'links' in result:
            links = result["links"]
            for link in links:
                items.append(self.get_element_details(
                    link, self.ELEMENT_TYPE["product"]))
        ...
        if 'collections' in result:
            ...
            if len(collections) > 0 and collections[0].get("collection_type") == "top_results":
                ...
                for collection in collections:
                    items.append(self.get_element_details(
                        collection, self.ELEMENT_TYPE["collection"]))

        if 'items' in result:
            result_items = result["items"]
            for result_item in result_items:
                items.append(self.get_element_details(
                    result_item, self.ELEMENT_TYPE["product"]))
